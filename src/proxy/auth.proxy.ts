...
export class AuthProxy {
  private readonly logger = new Logger(AuthProxy.name);
  ...
  constructor(
    ...
    this.apiUrl = this.config.getOrThrow('AUTH_API_URL');
  }

  /**
   * Busca dados da API Auth
  ...
  /**
   * Cria um novo recurso na API Auth
   * @param path - Caminho da API (sem a URL base)
  ...
  /**
   * Atualiza um recurso na API Auth
   * @param path - Caminho da API (sem a URL base)
  ...
  /**
   * Atualiza parcialmente um recurso na API Auth
   * @param path - Caminho da API (sem a URL base)
   * @param data - Dados para atualizar o recurso
   * @param config - Configurações adicionais do Axios
   */
  ...
  /**
   * Método base para encaminhar requisições para a API Auth
  ...
  protected async forward<T>(
    ...
    try {
      ...
      this.logger.error(
        `Auth API error: ${(error as Error).message}`,
